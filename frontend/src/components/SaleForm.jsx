...
const SaleForm = () => {
  ...
  const calculateRowTotal = (price, qty, unit, packSize) => {
    ...
    // 'loose' applies to tablets, single ampoules, etc.
    if (unit === 'loose') {
      effectivePrice = price / (packSize || 1); // Avoid division by zero
    }
  ...
  // Helper to toggle unit (Pack <-> Loose)
  const toggleUnit = (index, specificUnit = null) => {
    const newCart = [...cart];
    const item = newCart[index];
    
    if (specificUnit) {
        item.unit = specificUnit;
    } else {
        // Toggle between 'pack' and 'loose'
  ...
  // --- 🔥 UPDATED KEYBOARD SHORTCUTS (P / L) ---
  const handleQuantityKeyDown = (e, index) => {
    // Press 'L' for Loose (Tablet/Single)
    if (e.key.toLowerCase() === 'l') {
        e.preventDefault();
        toggleUnit(index, 'loose');
    } 
    // Press 'P' for Pack (Strip/Bottle/Box)
    else if (e.key.toLowerCase() === 'p') {
        e.preventDefault();
        toggleUnit(index, 'pack');
    }
  };
  ...
  const updateQty = (index, val) => {
    ...
    // Convert Loose to Pack fraction for stock checking
    const requiredStock = item.unit === 'loose' ? (newQty / item.packSize) : newQty;

    if (requiredStock > item.maxStock) {
       return alert(`⚠️ Stock Limit Exceeded!\nAvailable: ${item.maxStock} Packs\nYou tried: ${requiredStock.toFixed(2)} Packs`);
    }
    ...
    if (idx !== -1) {
      ...
      setCart([
        ...cart,
        {
          ...
          packSize: med.packSize || 1, // Avoid 0 div error
          unit: 'pack', // 🔥 Default is PACK
  ...
  const handleCheckout = async () => {
    ...
    if (isBillNeeded && !customer.name.trim()) return alert("Customer Name is required for Bill");
    ...
    const finalItems = cart.map((i) => {
        ...
        // Round to 4 decimals to be safe with DB math
    ...
    // --- 2. ADD DOSE CHARGE FOR BACKEND RECORD ---
    if (doseVal > 0) {
      finalItems.push({
        medicineId: null, name: "Medical/Dose Charge", batch: "-", expiry: null, hsn: "999", gst: 0,
        mrp: doseVal, price: doseVal, quantity: 1, total: doseVal, discount: 0
      });
    }
    ...
    try {
      ...
      if (isBillNeeded === true) {
        ...
        if (billWindow) billWindow.document.write("<h3>Generating Bill... Please wait.</h3>");
      ...
      setCart([]); setAmountGiven(""); setDoseAmount("");
        ...
        <div className="p-4 bg-gray-50 border-b border-gray-200">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-3">
              {cart.length > 0 && <button onClick={clearDraft} className="text-xs font-bold text-red-600 bg-red-50 px-3 py-1 rounded hover:bg-red-100 transition-colors">Clear Cart</button>}
          ...
          <div className="relative">
            <input className="w-full px-4 py-3 text-base border-2 border-gray-300 rounded-xl focus:outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/20 transition-all" placeholder="🔍 Scan barcode or type medicine name..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={handleKeyDown} autoFocus />
            {results.length > 0 && (
                    ...
                    <div key={med._id} onMouseDown={(e) => { e.preventDefault(); if (!isOutOfStock) addToCart(med); }}
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className={`font-bold ${isOutOfStock ? "text-gray-500" : "text-gray-800"}`}>{med.productName}</span>
                          {isOutOfStock && <span className="text-[10px] bg-gray-600 text-white px-1 rounded font-bold">OUT OF STOCK</span>}
                          {!isOutOfStock && isExpired && <span className="text-[10px] bg-red-600 text-white px-1 rounded font-bold">EXPIRED</span>}
                          {!isOutOfStock && !isExpired && isExpiringSoon && <span className="text-[10px] bg-orange-500 text-white px-1 rounded font-bold">NEAR EXP</span>}
                        </div>
                        {!isStaff && <div className="text-xs text-gray-500 mt-1 flex gap-3"><span className={`${isOutOfStock ? "text-gray-500 font-bold" : med.quantity < 10 ? "text-orange-600 font-bold" : "text-green-600"}`}>Stock: {med.quantity}</span><span>Batch: {med.batchNumber}</span><span className={`font-bold ${isExpired ? "text-red-600" : isExpiringSoon ? "text-orange-600" : "text-gray-500"}`}>Exp: {new Date(med.expiryDate).toLocaleDateString("en-IN", { month: "short", year: "numeric" })}</span></div>}
                      </div>
                      <div className="text-right"><div className={`font-bold text-lg ${isOutOfStock ? "text-gray-400" : "text-teal-600"}`}>₹{med.sellingPrice}</div></div>
                    </div>
                  );
                })}
              </div>
                  ...
                  <tr key={idx} className="hover:bg-gray-50 transition-colors group">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-gray-500 flex gap-2">
                        ...
                        <span className="text-teal-600 bg-teal-50 px-1 rounded">Pack of {item.packSize}</span>
                      </div>
                    </td>
                    {!isStaff && <td className="px-2 py-3 text-center text-gray-400 text-sm">₹{item.mrp}</td>}
                    {!isStaff && (
                      <td className="px-2 py-3 text-center">
                        <input type="number" value={item.discount} onChange={(e) => handleDiscountChange(idx, e.target.value)} className={`w-14 p-1 text-center text-sm border rounded outline-none focus:ring-2 focus:ring-teal-500 ${discWarning ? "border-red-500 text-red-600 bg-red-50" : "border-gray-300"}`} />
                      </td>
                    )}
                    <td className="px-2 py-3 text-center relative">
                      <input type="number" value={item.price} onChange={(e) => handleRateChange(idx, e.target.value)} className={`w-16 p-1 text-center text-sm font-bold border rounded outline-none focus:ring-2 focus:ring-teal-500 ${lossWarning ? "border-red-500 text-red-600 bg-red-50" : "border-gray-300"}`} />
                      {(lossWarning || discWarning) && <div className="absolute top-full left-1/2 -translate-x-1/2 bg-red-100 border border-red-200 text-red-600 text-[9px] px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap mt-1 font-bold z-20 flex flex-col items-center">{lossWarning && <span>⚠️ Below CP</span>}{!isStaff && discWarning && <span>⚠️ High Disc</span>}</div>}
                    </td>
                    
                    {/* --- 🔥 NEW UI & LOGIC FOR FAST QTY/UNIT --- */}
                    <td className="px-2 py-3 text-center">
                      <div className="flex flex-col items-center gap-1">
                        
                        {/* Qty Input with Key Listeners */}
                        <div className="flex items-center justify-center border border-gray-300 rounded-lg overflow-hidden w-28 shadow-sm bg-white">
                            ...
                            <input 
                                ...
                                title="Press 'P' for Pack, 'L' for Loose"
                            />
                            <button onClick={() => updateQty(idx, item.quantity + 1)} className="px-2 py-1 bg-gray-50 hover:bg-gray-200 text-gray-600 font-bold">+</button>
                        </div>

                        {/* Clickable Badge to Toggle Unit */}
                        <span 
                            onClick={() => toggleUnit(idx)}
                            className={`text-[10px] font-bold px-2 py-0.5 rounded cursor-pointer uppercase select-none transition-colors border
                                ${item.unit === 'loose' 
                    ...
                    <td className="px-4 py-3 text-right">
                        ...
                        {item.unit === 'loose' && (<div className="text-[10px] text-gray-400">(@ ₹{(item.price/item.packSize).toFixed(2)}/unit)</div>)}
                    </td>
      ...
      <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-fit sticky top-4">
        <div className="bg-gradient-to-br from-teal-600 to-teal-800 rounded-xl p-6 text-center text-white shadow-lg mb-4">
          ...
          <div className="mt-4 flex items-center justify-center gap-2 bg-teal-800/50 p-2 rounded-lg border border-teal-500/30">
            ...
            <input type="number" placeholder="0" value={doseAmount} onChange={(e) => setDoseAmount(e.target.value)} className="w-20 bg-white text-teal-900 font-bold text-center rounded px-1 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
          </div>
        </div>
        {lastSale && <div className={`mb-4 border rounded-lg p-3 flex justify-between items-center animate-pulse-once ${lastSale.isBillRequired ? "bg-green-50 border-green-200" : "bg-orange-50 border-orange-200"}`}><span className={`font-bold text-sm ${lastSale.isBillRequired ? "text-green-700" : "text-orange-700"}`}>{lastSale.isBillRequired ? "✅ Sale Saved!" : "✅ Saved (No Bill Created)"}</span>{lastSale.isBillRequired && <button onClick={handleReprint} className="text-sm underline font-bold text-green-800 hover:text-green-900">Reprint Bill</button>}</div>}
        <div className="mb-6"><div className="flex justify-between items-center mb-2"><span className={labelClass}>Customer Details</span><label className="flex items-center gap-2 cursor-pointer text-xs font-bold text-teal-700"><input type="checkbox" checked={isBillNeeded} onChange={(e) => setIsBillNeeded(e.target.checked)} className="w-4 h-4 text-teal-600 rounded focus:ring-teal-500" /> Print Bill</label></div><div className="space-y-3"><input className={`${inputClass} ${isBillNeeded && !customer.name ? "border-red-400 bg-red-50" : ""}`} placeholder={isBillNeeded ? "Customer Name *" : "Customer Name (Optional)"} value={customer.name} onChange={(e) => setCustomer({ ...customer, name: e.target.value })} />{isBillNeeded && <div className="flex gap-3"><input className={inputClass} placeholder="Phone" value={customer.phone} onChange={e => setCustomer({ ...customer, phone: e.target.value })} /><input className={inputClass} placeholder="Dr. Ref" value={customer.doctor} onChange={e => setCustomer({ ...customer, doctor: e.target.value })} /></div>}</div></div>
        <div className="mb-6"><span className={labelClass}>Payment Mode</span><div className="flex gap-3 mt-2"><button className={`flex-1 py-3 rounded-lg font-bold text-sm border transition-all ${paymentMode === "Cash" ? "bg-teal-700 text-white border-teal-700 shadow-md" : "bg-white text-gray-600 border-gray-300 hover:bg-gray-50"}`} onClick={() => setPaymentMode("Cash")}>💵 Cash</button><button className={`flex-1 py-3 rounded-lg font-bold text-sm border transition-all ${paymentMode === "Online" ? "bg-teal-700 text-white border-teal-700 shadow-md" : "bg-white text-gray-600 border-gray-300 hover:bg-gray-50"}`} onClick={() => setPaymentMode("Online")}>📱 UPI / Online</button></div>{paymentMode === "Cash" && <div className="mt-4 bg-orange-50 border border-orange-100 rounded-lg p-3 flex justify-between items-center"><input type="number" placeholder="Given Amount" value={amountGiven} onChange={(e) => setAmountGiven(e.target.value)} className="w-28 p-2 text-sm border border-orange-200 rounded focus:outline-none focus:border-orange-400" /><div className="text-right"><div className="text-xs text-gray-500 uppercase font-bold">Return Change</div><div className={`text-xl font-bold ${changeToReturn < 0 ? "text-red-600" : "text-green-600"}`}>₹{changeToReturn.toFixed(0)}</div></div></div>}</div>
